...
%><%
    ...
    StringWriter serverPromotionsMap = null;
    ...
    if (commerceService == null) {
        ...
        CommerceHelper.writeCart(cartWriter, commerceSession, resource.getResourceResolver());
        ...
        if (commerceSession.supportsClientsidePromotionResolution()) {
            ...
            serverPromotionsMap = new StringWriter();
            TidyJSONWriter promotionsWriter = new TidyJSONWriter(serverPromotionsMap);
            PromotionManager pm = resourceResolver.adaptTo(PromotionManager.class);
            promotionsWriter.array();
            if (pm != null) {
                for (Promotion promo : pm.getAvailablePromotions(resource.getResourceResolver())) {
                    if(!"/libs/commerce/components/promotion/server".equals(promo.getType()))
                        continue;

                    ValueMap props = promo.getConfig();
                    String code = props.get("code", "");
                    if (StringUtils.isNotBlank(code) && !props.get("disabled", Boolean.FALSE)) {
                        promotionsWriter.object()
                        .key("segments").value(StringUtils.join(promo.getSegments(), ","))
                        .key("path").value(code)
                        .endObject();
                    }
                }
...
%>
        ...
        if (CQ_Analytics && CQ_Analytics.CartMgr) {
            ...
            CQ_Analytics.CartMgr.serverPromotionsMap = <%= serverPromotionsMap %>;
