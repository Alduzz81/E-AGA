...
%><%@ include file="/libs/foundation/global.jsp" %><%
    ...
    try {
        ...
        setPageInitialData(writer, sling, cPage, resourceResolver, slingRequest);
...
%>if (CQ_Analytics && CQ_Analytics.PageDataMgr) {
    CQ_Analytics.PageDataMgr.loadInitProperties(<%=buf%>, false);
    CQ_Analytics.PageDataMgr.init();
}<%!
    void setPageInitialData(JSONWriter writer,
                            SlingScriptHelper sling,
                            Page currentPage, ResourceResolver resourceResolver, SlingHttpServletRequest slingRequest) throws Exception {
        writer.object();
        if (currentPage != null) {
            long monthlyHits = 0;
            try {
            ...
            if ( topLevelPage!= null ){
                ...
                writer.key("siteRoot").value(topLevelPage.getPath());
            }
        }
